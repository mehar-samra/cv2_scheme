...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        # Do not evalue "expressions.rest" - this has the bracket
        ...
        new_lamb_proc = do_lambda_form(Pair(formals, body), env)
        env.define(symbol, new_lamb_proc)
        return symbol
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    while expressions is not nil:
        # last expression is a tail
        is_tail = expressions.rest is nil
        value = scheme_eval(expressions.first, env, is_tail)
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        curr_bind = bindings.first
        validate_form(curr_bind, 2, 2)
        names = Pair(curr_bind.first, names)
        val = scheme_eval(curr_bind.rest.first, env)
...
# Extend the special forms with OpenCV forms
from scheme_opencv import *
add_opencv_special_forms(SPECIAL_FORMS)
